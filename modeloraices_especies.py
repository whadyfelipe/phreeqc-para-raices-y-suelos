...
#transporte de varias especies con dictionaries
...
especies=['P','N','K']
dict_rhodim={}
dict_rhodim['P']=rhodim
dict_rhodim['N']=rhodim
dict_rhodim['K']=0
...
dict_cmat={}
for esp in especies:
    dict_cmat[esp]=np.zeros((n,ntsteps+1))
...
#                mu=D0*dD(S[i])*dS-Ks*dk(S[i])
            ...
            mu*dS+Fw_node[i]
        if i in set1:
            f[i]=-D0*D(S[i])/dx*(S[i+1]-S[i])+Ks*k(S[i])-Wdim
        if i in set3:
            f[i]=-D0*D(S[i])/dx*(S[i]-S[i-1])+Ks*k(S[i])
...
Smat[:,0]=S0
# for i in range(n):
#     u[i]=-D0*D(S0[i])*(S0[i]-S0[i-1])/dx+Ks*k(S0[i])

dict_c0={}
dict_c={}

dict_c0['P']=0.66*Km*np.ones(n)
dict_c0['N']=0.66*Km*np.ones(n)
dict_c0['K']=0.1*Km*np.ones(n)

for esp in especies:
    dict_cmat[esp][:,0]=dict_c0[esp][:]
...
#recover the velocity values just in case they're needed
    for i in range(n):
        u[i]=-D0*D(S[i])*(S[i]-S[i-1])/dx+Ks*k(S[i])
#Recover the Fw values at the root nodes        
    Fw_table=Fw(S)
#solution of concentration equations for several species
    for esp in especies:
        c0=dict_c0[esp]
        rhodim_especie=dict_rhodim[esp]
        res=fsolve(Fconcent,dict_c0[esp],args=(t),full_output=1,col_deriv=1,band=(1,1))
        print('Solution code for concentration equation',esp,res[2],res[3])
        print('=======================================================')
        dict_c[esp]=res[0]
        for i in range(n):
            if dict_c[esp][i]<=0 : dict_c[esp][i]=0  #edit negative values
        dict_c0[esp]=dict_c[esp]
        dict_cmat[esp][:,j+1]=dict_c[esp]
        
#save results
if not os.path.exists('resultados_richards_multispecie'):
    os.mkdir('resultados_richards_multispecie')

np.savetxt('resultados_richards_multispecie/S.txt',Smat)        
np.savetxt('resultados_richards_multispecie/t.txt',times)        

for esp in especies:
    file='resultados_richards_multispecie/c['+esp+'].txt'
    np.savetxt(file,dict_cmat[esp])        


for esp in especies:    
    plt.figure()  
    for i in range(0,ntsteps,1):
        plt.plot(zcoo,dict_cmat[esp][:,i]/Km)
    plt.ylabel('c ['+esp+']')
    plt.xlabel('y')        
